...
macro (AddOptimizeOption)
  ...
  elseif (TARGET_ARCH STREQUAL "FX10")
    set(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -DFX10 -Kfast -w -Xg")
    set(CMAKE_C_FLAGS "${CMAKE_C_FLAGS} -DFX10 -Kfast -w -Xg")
    set(CMAKE_Fortran_FLAGS "${CMAKE_Fortran_FLAGS} -Cpp -DFX10 -Kfast -Knooptmsg")
  ...
  elseif (TARGET_ARCH STREQUAL "FX100")
    set(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -Kfast -w -Xg")
    set(CMAKE_C_FLAGS "${CMAKE_C_FLAGS} -Kfast -w -Xg")
    set(CMAKE_Fortran_FLAGS "${CMAKE_Fortran_FLAGS} -Cpp -Kfast -Knooptmsg")
    if(enable_PreciseTimer)
      SET(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -Nfjcex")
    endif()
  ...
  elseif(USE_F_TCS STREQUAL "YES")	# (TARGET_ARCH STREQUAL "ITO_TCS") OR (TARGET_ARCH STREQUAL "F_TCS")
    set(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -Kfast -w -Xg -std=c++11")
    set(CMAKE_C_FLAGS "${CMAKE_C_FLAGS} -Kfast -w -Xg")
    set(CMAKE_Fortran_FLAGS "${CMAKE_Fortran_FLAGS} -Cpp -Kfast -Knooptmsg")
    if(enable_PreciseTimer)
      SET(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -Nfjcex")
  ...
  elseif(CMAKE_CXX_COMPILER_ID STREQUAL "PGI")
    set(CMAKE_CXX_FLAGS "${CMAKE_CXX_FLAGS} -fast -O3 -v -std=c++11")
